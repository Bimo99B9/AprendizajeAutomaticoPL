...
print("Accuracy en el conjunto de test: %0.4f" % metrics.accuracy_score(y_test, y_pred))
...
gs_nb = GridSearchCV(
    sys_nb,
    param_grid={"var_smoothing": val_smooth},
    scoring="accuracy",
    cv=folds,
    verbose=1,
    n_jobs=-1,
...
print(
    "Accuracy para cada valor durante la búsqueda: ",
    gs_nb_trained.cv_results_["mean_test_score"],
)
print("Mejor combinación de hiperparámetros: ", gs_nb_trained.best_params_)
print("Mejor rendimiento obtenido en GridSearch: %0.4f" % gs_nb_trained.best_score_)
y_pred = gs_nb_trained.predict(X_test)
print("Accuracy en conjunto de test: %0.4f" % metrics.accuracy_score(y_test, y_pred))
...
sys_nb_calib = CalibratedClassifierCV(gs_nb_trained, method="sigmoid", cv=folds)
